...
document.getElementById("sent--one").addEventListener("keypress", function(event) {
    if (event.keyCode === 13) { // Kiểm tra nút Enter đã được nhấn
        ...
        thuthichat__one(); // Gọi hàm thuthichat__two() khi nhấn nút Enter
        sendMessage__one(); // Gọi hàm sendMessage__two() để gửi tin nhắn (nếu cần)
    }
});

function thuthichat__one() {
    let a = 3;
    let b = 6;
    
    let tinNhan = document.getElementById("sent--one").value.toUpperCase(); 
    ...
    textarea.value = "Thuận Gửi (" + timeString + "): " + tinNhanGiaiMa + "\n" + textarea.value;
    textarea.scrollTop = textarea.scrollHeight; 

          // In tin nhắn đã giải mã ra textarea
      var nguoigui = document.getElementById("messageTextarea--one");
    ...
    _textarea.value = "Thuận Gửi (" + timeString + "): Mã Hóa: " + maMaTin + "\n" + _textarea.value;
    _textarea.value = "Thuận Gửi (" + timeString + "): Giải Mã: " + tinNhanGiaiMa + "\n" + _textarea.value;
    _textarea.value = "----------------------Thuận-----------------------" + "\n" + _textarea.value;
